...
export class ObjectStorageService {
  ...
  // Server-side upload — writes a buffer directly to the private bucket and
  // returns the canonical /objects/<id> path the rest of the system uses.
  async uploadBuffer(buffer: Buffer, contentType: string, prefix: string = "uploads"): Promise<string> {
    const privateObjectDir = this.getPrivateObjectDir();
    const objectId = randomUUID();
    const fullPath = `${privateObjectDir.replace(/\/$/, "")}/${prefix}/${objectId}`;
    const { bucketName, objectName } = parseObjectPath(fullPath);
