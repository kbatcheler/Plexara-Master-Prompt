...
-   **Authoritative Drug/Supplement DBs**: Integrates RxNorm, OpenFDA, NIH-ODS-cited supplement catalog, NIH RxTerms (drug autocomplete) and NIH DSLD (supplement-ingredient autocomplete) via `lib/medical-databases.ts` and the `/api/lookup/{medications,rxterms,supplements,dsld,adverse-events}` endpoints. Frontend `NihAutocompleteInput.tsx` provides debounced (300 ms), abortable, free-text-fallback combobox suggestions; selecting an RxTerms hit persists the canonical `rxNormCui` on the medication row.
-   **Stack Intelligence**: Synchronous `POST /api/patients/:patientId/supplements/stack-analysis` endpoint (operation `analyseSupplementStack`) critiques the patient's CURRENT supplement + medication stack (form, dose, timing, interactions, gaps, redundancies) using `lib/stack-analysis-ai.ts` (LLM_MODELS.utility, withLLMRetry, parseJSONFromLLM, stripPII, buildDemographicBlock). Pulls active supplements + meds + latest reconciled biomarker findings + biomarker history (excl derived, deduped by name, capped at 40) + pharmacogenomics evidence from `evidenceRegistryTable` and falls back to `patientsTable.medications` jsonb when the structured `medicationsTable` is empty (Health Profile data-flow merge). Frontend (`Supplements.tsx` → `StackAnalysisPanel`) renders overall assessment, item verdicts with biomarker/genetics chips, gaps, interactions, a 6-slot timing grid + notes, pill burden and monthly cost; an inline amber "stack changed" banner appears after any add/edit/delete to either supplements or medications and prompts a one-click re-run. Lens enrichment (`enrichment.ts → loadActiveSupplements`) now also attaches active supplements to the lens payload (additive — only when non-empty).
-   **Care Plan in Comprehensive Report**: `reports-ai.ts` builds a unified `CURRENT CARE PLAN` block from active supplements + medications and instructs the LLM to add a Current Care Plan Assessment. `ComprehensiveReportInput` gained an optional `currentMedications` field; both report-generation entry points (`routes/comprehensive-report.ts` POST `/` and `lib/post-interpretation-orchestrator.ts`) load active medications and pass them through. No existing response shape changes.
-   **Recommended Next Tests Card**: `IntelligenceSummary.tsx` surfaces `report.followUpTesting` from the latest comprehensive report as a dashboard card (hidden when empty) — reuses the existing `/comprehensive-report/latest` payload, no new backend field.
-   **Med-Rules Engine Data Flow**: The depletion-rules engine in `orchestrator-intelligence.ts` intentionally reads only structured `medicationsTable` rows (each rule keys off `drugClass`); patients who only filled the Health Profile free-text meds therefore get no depletion alerts here by design — those meds still reach the lens enrichment via PatientContext and are merged into Stack Intelligence (which tolerates missing drugClass).
...
-   **Modularity & Production Hardening**: `lib/ai.ts` and `routes/records.ts` refactored into focused modules and sub-routers. `lib/records-processing.ts` now delegates LLM-payload enrichment to `lib/enrichment.ts`, three-lens dispatch + degradation to `lib/lens-dispatch.ts`, and atomic interpretation persistence to `lib/interpretation-persist.ts`. `lib/post-interpretation-orchestrator.ts` delegates Steps 1c–1g (ratios, patterns, drug-depletion alerts, multi-panel domain delta, longitudinal learning) to `lib/orchestrator-intelligence.ts`. `routes/protocols.ts` is a thin shim that composes `protocols-shared.ts` (REFERENCE_PROTOCOLS + helpers), `protocols-browse.ts` (GETs + globalRouter), and `protocols-adoption.ts` (POST/generate, GET/POST/PATCH adoptions). Production boot guard for `SESSION_SECRET`, feature-flag gating, and sensitive-action rate limiter added (now also covers `report-export`).
...
-   **Universal Evidence Map**: `evidence_registry` table (one row per uploaded record, UNIQUE on `record_id`) makes non-blood-panel records (DEXA, cancer screening, pharmacogenomics, specialized panels) visible to the intelligence layer. Populated by `records-processing.ts` after extraction (idempotent upsert via `onConflictDoUpdate`); attached to lens enrichment via `enrichment.ts` (non-current non-blood evidence on `anonymisedForLens.additionalEvidence`); fed to the comprehensive report as `evidenceMap` and surfaced back as a deterministic `evidenceBase` array on `ComprehensiveReportOutput`. Orchestrator marks ONLY the record IDs in the input snapshot as `integratedIntoReport=true` (post-snapshot uploads stay pending). New `GET /api/patients/:id/evidence` endpoint (auth + ownership) backs an `EvidenceMap.tsx` chronological timeline above Recent Records on the dashboard. Frontend Records / UploadZone dropdowns gained `dexa_scan`, `cancer_screening`, `specialized_panel` options. Extraction prompt routing in `extraction.ts` tightens the imaging guard to exclude DEXA-style record types so `dexa_scan` reaches the dedicated DEXA prompt branch.
-   **Curriculum-Style /help System**: `pages/Help.tsx` orchestrates a deep functional-medicine guide via `HelpLayout` (sticky sidebar TOC + `IntersectionObserver` scroll-spy + `aria-current="location"` on the active TOC link + mobile dropdown). Section primitives live in `components/help/` (`HelpSection`, `ClinicalDetail` collapsible expander, `OpenFeature` deep-link button, `HelpHint` Radix tooltip with `?` glyph that anchor-links to `/help#<id>`). Section content is split into `pages/help/` modules (`Overview`, `FunctionalMedicinePrimer`, `HealthDomainsGuide` covering all 8 domains with biomarkers/patterns/lifestyle/supplements/medications, `FeatureGuide` with per-page deep links, `GaugesAndColors`, `PrivacyData`, `WhenToCallDoctor`, `Faq`, `Contact`). Inline `HelpHint` tooltips wired on Dashboard "System domains" → `health-domains`, BiologicalAge h1 → `feature-biological-age`, Trends h1 → `feature-trends`, Report comprehensive heading → `feature-comprehensive-report`, Genetics h1 → `feature-genetics`, Supplements Stack Intelligence card → `feature-supplements-stack-intel`. Anchor IDs are append-only by contract to keep deep links stable.
